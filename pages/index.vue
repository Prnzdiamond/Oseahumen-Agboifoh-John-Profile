        ...
        <div class="text-center lg:text-left space-y-8 animate-slide-up">
          ...
          <div v-else-if="ownerStore.error" class="space-y-6">
            ...
            <div class="flex gap-4">
              <NuxtLink to="/projects" class="btn-primary animate-bounce-in">View My Work</NuxtLink>
              <button @click="ownerStore.fetchOwner()" class="btn-secondary animate-bounce-in-delayed">Retry</button>
            </div>
          </div>
        </div>

        <!-- Enhanced Hero Visual — ORIGINAL, unchanged -->
...
const techPageCount = computed(() => {
  ...
  return total ? Math.ceil(total / PILLS_PER_PAGE) : 1
})

const currentTechPage = computed(() => {
  const techs = ownerStore.owner?.tech_stack ?? []
  if (!techs.length) return []
  const start = (techPage.value % techPageCount.value) * PILLS_PER_PAGE
  return techs.slice(start, start + PILLS_PER_PAGE)
})

const startTechCycle = () => {
  techTimer = setInterval(() => { techPage.value++ }, 2000)
}

onUnmounted(() => { if (techTimer) clearInterval(techTimer) })
  ...
  const type = () => {
    if (!isDeleting.value) {
      if (typewriterIndex.value < headline.length) {
        ...
        setTimeout(() => { isDeleting.value = true; type() }, pauseTime)
      }
    } else {
