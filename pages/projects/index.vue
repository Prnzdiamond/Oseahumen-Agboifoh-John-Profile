      ...
      <div class="relative max-w-6xl mx-auto px-6">
        ...
        <div class="flex flex-col md:flex-row gap-4 justify-between items-center mb-4 animate-slide-up-delayed">
          <div class="relative flex-1 max-w-md">
            <input v-model="searchQuery" type="text" placeholder="Search projects..." class="w-full px-4 py-3 pl-12 bg-white dark:bg-gray-800 border border-blue-200
          ...
          <div class="flex items-center gap-3">
            ...
            <button @click="filterPanelOpen = !filterPanelOpen" :class="[
            ...
            <select v-model="sortBy" class="px-4 py-3 bg-white dark:bg-gray-800 border border-blue-200 dark:border-gray-600
            ...
            <button @click="viewMode = viewMode === 'grid' ? 'list' : 'grid'" class="p-3 bg-white dark:bg-gray-800 border border-blue-200 dark:border-gray-600
                     rounded-xl hover:bg-blue-50 dark:hover:bg-gray-700 transition-all duration-200">
              <svg v-if="viewMode === 'grid'" class="w-5 h-5 text-gray-600 dark:text-gray-400" fill="none"
                stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <transition enter-active-class="transition duration-200 ease-out" enter-from-class="opacity-0 -translate-y-2"
          ...
          <div v-if="filterPanelOpen" class="bg-white dark:bg-gray-800 rounded-2xl border border-blue-200 dark:border-gray-600
                      shadow-xl p-5 mb-2">
            ...
            <div v-if="totalActiveFilters" class="flex flex-wrap items-center gap-2 mb-5 pb-4
              ...
              <!-- Tech chips -->
              <button v-for="slug in effectiveTechFilters" :key="`t-${slug}`" @click="removeTechFilter(slug)" class="inline-flex items-center gap-1.5 px-3 py-1 bg-blue-600 text-white
              ...
              <!-- Context chips -->
              <button v-for="slug in effectiveCtxFilters" :key="`c-${slug}`" @click="removeCtxFilter(slug)" class="inline-flex items-center gap-1.5 px-3 py-1 bg-indigo-600 text-white
              ...
              <button @click="clearAllFilters" class="text-xs text-gray-400 hover:text-red-500 dark:hover:text-red-400
            ...
            <div class="space-y-5">
                ...
                <div class="space-y-3">
                  <div v-for="group in groupedTechs" :key="group.category">
                    ...
                    <div class="flex flex-wrap gap-2">
                      <button v-for="tech in group.techs" :key="tech.slug" @click="toggleTechFilter(tech.slug)" :class="[
                        ...
                        <img v-else-if="tech.custom_icon_url" :src="tech.custom_icon_url" :alt="tech.name"
              ...
              <!-- Divider -->
              <div
                v-if="groupedTechs.length && (filterStore.meta.industries.length || filterStore.meta.types.length || filterStore.meta.tags.length)"
              ...
              <div v-if="filterStore.meta.industries.length">
                ...
                <div class="flex flex-wrap gap-2">
                  <button v-for="ind in filterStore.meta.industries" :key="ind" @click="toggleCtxFilter(ind)" :class="[
                    'inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-sm font-medium',
                    'border transition-all duration-200 hover:scale-105',
              ...
              <div v-if="filterStore.meta.types.length">
                ...
                <div class="flex flex-wrap gap-2">
                  <button v-for="type in filterStore.meta.types" :key="type"
                    @click="toggleCtxFilter(type.replace(/_/g, '-'))" :class="[
                      'inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-sm font-medium',
                      'border transition-all duration-200 hover:scale-105',
              ...
              <div v-if="filterStore.meta.tags.length">
                ...
                <div class="flex flex-wrap gap-2">
                  <button v-for="tag in filterStore.meta.tags" :key="tag" @click="toggleCtxFilter(tag)" :class="[
                    'inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-sm font-medium',
                    'border transition-all duration-200 hover:scale-105',
      ...
      <div class="max-w-7xl mx-auto px-6">
        ...
        <div v-if="!projectStore.loading" class="mb-6 flex flex-wrap items-center justify-between gap-3">
          ...
          <div class="flex items-center gap-3">
            ...
            <button v-if="!filterStore.isViewingAll && filterStore.hasSessionFilter && !totalActiveFilters"
              @click="filterStore.viewAll()" class="text-xs text-blue-600 dark:text-blue-400 hover:underline">
              ← Re-apply saved filter
            </button>
            <button v-if="filterStore.isViewingAll" @click="filterStore.isViewingAll = false"
        ...
        <div v-if="!projectStore.loading && sortedProjects.length"
          ...
          <div v-for="(project, index) in sortedProjects" :key="project.id"
            :style="{ animationDelay: `${index * 0.08}s` }" class="group animate-fade-in-up">
            <!-- Grid card -->
            <div v-if="viewMode === 'grid'" class="bg-white dark:bg-gray-800 rounded-2xl shadow-lg hover:shadow-2xl
              ...
              <div class="relative h-48 overflow-hidden flex-shrink-0">
                <img :src="project.cover_image || '/images/default_pro_cover.png'" :alt="project.title"
                ...
                <div class="absolute top-3 right-3 flex -space-x-1">
                  <template v-for="tech in project.technologies?.slice(0, 2)" :key="tech">
                    <div v-if="getIconClass(tech)" class="w-7 h-7 bg-white dark:bg-gray-800 rounded-full flex items-center justify-center
              ...
              <div class="p-6 flex flex-col flex-1">
                ...
                <p class="text-gray-600 dark:text-gray-400 mb-4 line-clamp-2 flex-1 text-sm">{{ project.description }}
                </p>
                <div class="flex flex-wrap gap-2 mt-auto">
                  <span v-for="tech in project.technologies?.slice(0, 4)" :key="tech" class="inline-flex items-center gap-1.5 px-3 py-1
            ...
            <!-- List card -->
            <div v-else class="bg-white dark:bg-gray-800 rounded-2xl shadow-lg hover:shadow-xl
                ...
                <div class="flex-1 p-6">
                  <div class="flex justify-between items-start gap-3 mb-2">
                    ...
                    <div class="flex gap-2 flex-shrink-0">
                      <NuxtLink :to="`/projects/${project.slug}`"
                        class="px-3 py-1 bg-blue-600 text-white rounded-lg text-sm font-medium hover:bg-blue-700">
                        Details
                      </NuxtLink>
                      <a v-if="project.demo_url" :href="project.demo_url" target="_blank"
                  ...
                  <div class="flex flex-wrap gap-2">
                    <span v-for="tech in project.technologies?.slice(0, 5)" :key="tech" class="inline-flex items-center gap-1.5 px-2 py-1 text-xs font-medium rounded
                             bg-gradient-to-r from-blue-50 to-cyan-50 dark:from-blue-900/30 dark:to-cyan-900/30
