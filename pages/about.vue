    ...
    <section
      ...
      <div class="relative max-w-6xl mx-auto px-6">
        ...
        <div v-else-if="ownerStore.owner" class="text-center animate-fade-in">
          <!-- Avatar with OrbitingIcons - responsive sizing -->
          <div class="relative inline-flex items-center justify-center mb-8 px-4 sm:px-0"
            style="width: clamp(260px, 90vw, 340px); height: clamp(260px, 90vw, 340px);">
            <!-- Orbiting tech icons fill the outer space -->
      ...
      <div class="max-w-6xl mx-auto px-6">
        <div class="grid lg:grid-cols-3 gap-12">
          ...
          <div class="lg:col-span-2 space-y-8">
            ...
            <div v-if="ownerStore.owner.professional_journey?.length" class="animate-slide-up-delayed">
              <h3 class="text-2xl font-bold text-gray-900 dark:text-white mb-6 flex items-center gap-3">
                <div
                  class="w-6 h-6 bg-gradient-to-r from-blue-400 to-cyan-400 rounded-md flex items-center justify-center flex-shrink-0">
                      ...
                      <div
                        ...
                        <div class="flex items-start justify-between mb-2">
                          ...
                          <span v-if="job.is_current"
                            class="px-3 py-1 bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-200 text-xs rounded-full animate-pulse">Current</span>
                        </div>
              ...
              <div class="space-y-6">
                ...
                <div v-if="frontendTech.length"
                  ...
                  <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
                    <div v-for="(tech, index) in frontendTech" :key="tech.technology"
                      ...
                      <!-- Tooltip -->
                      <div
                        class="absolute -top-10 left-1/2 -translate-x-1/2 px-3 py-1 bg-gray-900 dark:bg-gray-700 text-white text-xs rounded-lg opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none whitespace-nowrap z-10">
                        {{ tech.technology }}<span v-if="tech.rating"> • {{ tech.rating }}/5★</span>
                        <div
                          class="absolute top-full left-1/2 -translate-x-1/2 w-0 h-0 border-l-4 border-r-4 border-t-4 border-transparent border-t-gray-900 dark:border-t-gray-700">
                        </div>
                      </div>
                    </div>
                  </div>
                ...
                <div v-if="backendTech.length"
                  ...
                  <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
                    <div v-for="(tech, index) in backendTech" :key="tech.technology"
                      :style="{ animationDelay: `${index * 0.05}s` }"
                      class="group relative p-4 bg-gradient-to-r from-blue-50 to-cyan-50 dark:from-blue-900/30 dark:to-cyan-900/30 rounded-lg border border-blue-200 dark:border-blue-700 hover:shadow-md hover:scale-105 transition-all duration-300 animate-fade-in-up cursor-pointer">
                      <div class="text-center">
                        <div class="flex justify-center mb-2">
                ...
                <!-- Database & Tools -->
                <div v-if="databaseAndTools.length"
                  class="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-lg border border-blue-100 dark:border-gray-700">
                  <h4 class="text-lg font-semibold text-gray-900 dark:text-white mb-4 flex items-center gap-2">
                  ...
                  <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
                    <div v-for="(tech, index) in databaseAndTools" :key="tech.technology"
                      :style="{ animationDelay: `${index * 0.05}s` }"
                      class="group relative p-4 bg-gradient-to-r from-blue-50 to-cyan-50 dark:from-blue-900/30 dark:to-cyan-900/30 rounded-lg border border-blue-200 dark:border-blue-700 hover:shadow-md hover:scale-105 transition-all duration-300 animate-fade-in-up cursor-pointer">
                      <div class="text-center">
                        <div class="flex justify-center mb-2">
                ...
                <div v-if="otherTech.length"
                  ...
                  <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
                    <div v-for="(tech, index) in otherTech" :key="tech.technology"
                      :style="{ animationDelay: `${index * 0.05}s` }"
                      class="group relative p-4 bg-gradient-to-r from-blue-50 to-cyan-50 dark:from-blue-900/30 dark:to-cyan-900/30 rounded-lg border border-blue-200 dark:border-blue-700 hover:shadow-md hover:scale-105 transition-all duration-300 animate-fade-in-up cursor-pointer">
                      <div class="text-center">
                        <div class="flex justify-center mb-2">
          ...
          <div class="space-y-8">
                ...
                <div class="space-y-3">
                  <div v-for="(hobby, index) in ownerStore.owner.hobbies" :key="hobby.name"
                    ...
                    <div class="flex items-center gap-4">
                      <div
                        class="w-12 h-12 bg-gradient-to-r from-blue-500 to-cyan-500 rounded-xl flex items-center justify-center flex-shrink-0 group-hover:scale-110 transition-transform duration-300">
                        <!-- Real Lucide icon if it's a valid name, emoji fallback for legacy data -->
                        <LucideIcon v-if="hobby.icon && isLucideName(hobby.icon)" :name="hobby.icon"
                          class="text-white w-6 h-6" :stroke-width="2" />
                        <span v-else class="text-white text-lg">{{ hobby.icon || '🎯' }}</span>
                      </div>
                      <div class="flex-1 min-w-0">
                        <div
                          class="font-medium text-gray-900 dark:text-white group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors duration-300">
                          {{ hobby.name }}</div>
            ...
            <div v-if="connectLinks.length" class="animate-slide-up-delayed">
              <h3 class="text-xl font-bold text-gray-900 dark:text-white mb-4 flex items-center gap-2">
                <div class="w-5 h-5 bg-gradient-to-r from-cyan-400 to-blue-400 rounded flex-shrink-0"></div>
                Connect
              </h3>
              <div
                class="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-lg border border-blue-100 dark:border-gray-700">
                <div class="space-y-3">
                  <a v-for="link in connectLinks" :key="link.platform" v-show="link.url"
                    :href="buildPlatformUrl(link.platform, link.url)" target="_blank" rel="noopener noreferrer"
                    ...
                    <div
                      class="w-9 h-9 bg-gradient-to-r from-blue-100 to-cyan-100 dark:from-blue-900/40 dark:to-cyan-900/40 rounded-lg flex items-center justify-center group-hover:scale-110 transition-transform duration-200 flex-shrink-0">
                      <!-- Devicon for known platforms -->
                      <i v-if="getDeviconClass(link.platform)" :class="getDeviconClass(link.platform)"
                        class="text-lg leading-none"></i>
                      <!-- Lucide fallback for platforms not in Devicon -->
                      <LucideIcon v-else :name="getLucideIcon(link.platform)"
                    ...
                    <div class="flex-1 min-w-0">
                      <div
                        ...
                        {{ getPlatformLabel(link.platform) }}</div>
                      <div class="text-sm text-gray-500 dark:text-gray-400 truncate">{{
                        getPlatformDescription(link.platform) }}</div>
                    </div>
                    <LucideIcon name="arrow-up-right"
                      class="w-4 h-4 text-gray-400 group-hover:text-blue-500 transition-colors duration-200 flex-shrink-0" />
                  </a>
                </div>
              </div>
            </div>
            ...
            <div class="animate-slide-up-delayed">
              <div
                ...
                <div class="relative z-10">
                  ...
                  <p class="text-blue-100 text-sm mb-4">Currently accepting new freelance projects and collaboration
                    opportunities.</p>
                  <NuxtLink to="/contact"
...
// FIX-5: owner.links is the correct API key (not owner.urls).
// resolveConnectLinks falls back to social_links for older API responses.
const connectLinks = computed(() => resolveConnectLinks(ownerStore.owner))
