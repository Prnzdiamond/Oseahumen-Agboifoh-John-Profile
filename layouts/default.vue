<template>
    <div class="min-h-screen flex flex-col bg-white dark:bg-gray-900 transition-colors duration-300">
      ...
      <header class="sticky top-0 z-50 bg-white/80 dark:bg-gray-900/80 backdrop-blur-xl border-b border-blue-100/50 dark:border-gray-700/50 transition-all duration-300">
          ...
          <div class="flex justify-between items-center h-16">
            ...
            <nav class="hidden lg:flex items-center space-x-1">
              <NuxtLink v-for="link in navLinks" :key="link.to" :to="link.to" 
                        class="relative px-4 py-2 rounded-lg text-gray-700 dark:text-gray-300 hover:text-blue-600 dark:hover:text-blue-400 font-medium transition-all duration-200 group">
                <span class="relative z-10">{{ link.name }}</span>
                <div class="absolute inset-0 bg-gradient-to-r from-blue-50 to-cyan-50 dark:from-blue-900/20 dark:to-cyan-900/20 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity duration-200"></div>
                <!-- Active indicator -->
                <div class="absolute bottom-0 left-1/2 transform -translate-x-1/2 w-0 h-0.5 bg-gradient-to-r from-blue-500 to-cyan-500 group-hover:w-full transition-all duration-300 router-link-active:w-full"></div>
              </NuxtLink>
            </nav>
        ...
        <div :class="{ 'translate-x-0 opacity-100': mobileMenuOpen, 'translate-x-full opacity-0': !mobileMenuOpen }" 
             class="lg:hidden fixed top-16 right-0 w-80 h-[calc(100vh-4rem)] bg-white/95 dark:bg-gray-900/95 backdrop-blur-xl shadow-2xl transform transition-all duration-300 ease-out z-50 border-l border-blue-100 dark:border-gray-700">
          
          <!-- Mobile Nav Header -->
          <div class="p-6 border-b border-blue-100 dark:border-gray-700">
            <div class="flex items-center space-x-3">
              <div class="w-8 h-8 bg-gradient-to-br from-blue-500 to-cyan-500 rounded-lg flex items-center justify-center overflow-hidden">
                <img v-if="owner?.avatar" 
                     :src="owner.avatar" 
                     :alt="owner.name"
              ...
              <div>
                <div class="font-semibold text-gray-900 dark:text-white">{{ owner?.name || 'Portfolio' }}</div>
          ...
          <nav class="p-6 space-y-2">
            <NuxtLink v-for="(link, index) in navLinks" :key="link.to" :to="link.to" 
                      ...
                      class="flex items-center space-x-3 py-3 px-4 rounded-xl text-gray-700 dark:text-gray-300 hover:bg-gradient-to-r hover:from-blue-50 hover:to-cyan-50 dark:hover:from-blue-900/20 dark:hover:to-cyan-900/20 hover:text-blue-600 dark:hover:text-blue-400 font-medium transition-all duration-200 group animate-slide-in">
              <component :is="link.icon" class="w-5 h-5 group-hover:scale-110 transition-transform duration-200" />
              <span>{{ link.name }}</span>
              <div class="ml-auto w-2 h-2 rounded-full bg-gradient-to-r from-blue-500 to-cyan-500 opacity-0 group-hover:opacity-100 transition-opacity duration-200"></div>
            </NuxtLink>
          </nav>
  
          <!-- Mobile Nav Footer -->
          <div class="absolute bottom-0 left-0 right-0 p-6 border-t border-blue-100 dark:border-gray-700">
            <div class="text-center">
              ...
              <div class="flex justify-center space-x-3">
                <a v-if="owner?.social_links?.github" 
                   ...
                   class="w-8 h-8 bg-gradient-to-r from-blue-100 to-cyan-100 dark:from-blue-900/30 dark:to-cyan-900/30 rounded-lg flex items-center justify-center hover:scale-110 transition-transform duration-200">
      ...
      <footer class="bg-gradient-to-r from-gray-50 to-blue-50/30 dark:from-gray-800 dark:to-gray-900 border-t border-blue-100/50 dark:border-gray-700/50 mt-auto">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
          <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
            ...
            <div class="space-y-4">
              <div class="flex items-center space-x-3">
                <div class="w-10 h-10 bg-gradient-to-br from-blue-600 to-cyan-500 rounded-xl flex items-center justify-center overflow-hidden">
                  <img v-if="owner?.avatar" 
                       :src="owner.avatar" 
                       :alt="owner.name"
                       class="w-full h-full object-cover rounded-xl" />
                ...
                <div>
                  <div class="font-bold text-gray-900 dark:text-white">{{ owner?.name || 'Portfolio' }}</div>
                  <div class="text-sm text-gray-500 dark:text-gray-400">{{ owner?.headline || 'Professional Portfolio' }}</div>
                </div>
              </div>
              <p class="text-gray-600 dark:text-gray-400 text-sm leading-relaxed">
                {{ owner?.bio || 'Crafting exceptional digital experiences with modern technologies and innovative solutions.' }}
              </p>
            </div>
            ...
            <div class="space-y-4">
              ...
              <div class="space-y-2">
                <NuxtLink v-for="link in navLinks" :key="link.to" :to="link.to"
                          class="block text-gray-600 dark:text-gray-400 hover:text-blue-600 dark:hover:text-blue-400 text-sm transition-colors duration-200">
                  {{ link.name }}
                </NuxtLink>
              </div>
            </div>
            ...
            <div class="space-y-4">
              ...
              <div class="space-y-3">
                ...
                <div v-if="owner?.contact_methods?.email" class="flex items-center space-x-2 text-sm text-gray-600 dark:text-gray-400">
                  ...
                  <a :href="`mailto:${owner.contact_methods.email}`" class="hover:text-blue-600 dark:hover:text-blue-400 transition-colors">
          ...
          <!-- Footer Bottom -->
          <div class="mt-8 pt-8 border-t border-blue-100 dark:border-gray-700 flex flex-col md:flex-row justify-between items-center space-y-4 md:space-y-0">
            <div class="text-sm text-gray-500 dark:text-gray-400">
              &copy; {{ new Date().getFullYear() }} {{ owner?.name || 'Portfolio' }}. Built with ❤️ using Nuxt & Laravel
            </div>
            <div class="flex items-center space-x-4 text-sm text-gray-500 dark:text-gray-400">
              <span class="flex items-center space-x-1">
                <div class="w-2 h-2 bg-gradient-to-r from-blue-500 to-cyan-500 rounded-full animate-pulse"></div>
                <span>{{ getRandomTechStack() }}</span>
              </span>
            </div>
          </div>
        </div>
