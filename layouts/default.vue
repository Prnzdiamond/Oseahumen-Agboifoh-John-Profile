<template>
  <div class="min-h-screen flex flex-col bg-white dark:bg-gray-900 transition-colors duration-300">
    ...
    <header class="sticky top-0 z-50 bg-white/80 dark:bg-gray-900/80 backdrop-blur-xl border-b border-blue-100/50 dark:border-gray-700/50 transition-all duration-300">
      ...
      <!-- Enhanced Mobile Navigation - Fixed Width Issue -->
      <div :class="{ 'translate-x-0 opacity-100 visible': mobileMenuOpen, 'translate-x-full opacity-0 hidden': !mobileMenuOpen }" 
     class="lg:hidden fixed top-16 right-0 w-full max-w-sm sm:w-80 h-[calc(100vh-4rem)] bg-white/95 dark:bg-gray-900/95 backdrop-blur-xl shadow-2xl transform transition-all duration-300 ease-out z-50 border-l border-blue-100 dark:border-gray-700">
  ...
  <nav class="p-4 sm:p-6 space-y-2 overflow-y-auto flex-1">
    <NuxtLink v-for="(link, index) in navLinks" :key="link.to" :to="link.to" 
      ...
      <div class="ml-auto w-2 h-2 rounded-full bg-gradient-to-r from-blue-500 to-cyan-500 opacity-0 group-hover:opacity-100 transition-opacity duration-200 flex-shrink-0"></div>
    </NuxtLink>
  </nav>
          ...
          <div class="space-y-4">
            <div class="flex items-center space-x-3">
              ...
              <div class="min-w-0 flex-1">
                ...
                <div class="text-sm text-gray-500 dark:text-gray-400 truncate">{{ owner?.headline || 'Professional Portfolio' }}</div>
              </div>
            </div>
